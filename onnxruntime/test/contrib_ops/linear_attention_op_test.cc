        ...
        if (!inverse_gqa) {
          ...
          for (int g = 0; g < heads_per_group; g++) {
            ...
            for (int v_idx = 0; v_idx < head_dim_v; v_idx++) {
              ...
              // Standard GQA: output head == q head, indexed with out_heads stride.
              int out_idx = ((b * out_heads + (kv_h * heads_per_group + g)) * seq_length + t) * head_dim_v + v_idx;
              output[out_idx] = scale * sum;
            }
          }
        } else {
...
void RunLinearAttentionGQATest(
  ...
  // Output head count is max(q_num_heads, kv_num_heads), matching the schema.
  int out_heads = std::max(q_num_heads, kv_num_heads);
